#!/usr/bin/env node
/**
 * perturb.cjs — A3 최종 게이트: 섭동 매트릭스 (2026-07-21)
 * 기준+프리셋 5종을 각각 전체 gen 체인(parameter→primitive→semantic→component)에 통과시키고,
 * 8종 컴포넌트의 191검사가 모든 프리셋에서 ✗0인지 확인한다. "값이 아니라 값을 뽑는 엔진"의 증명.
...
for (const p of PRESETS) {
  ...
  rows.push({ id:p.id, name:p.name, brand:p.brandFrom, neutral:p.neutral,
    ...
    fails: fails.map(f=>`[${f.id}] ${f.msg}`), semErr, compErr, sample });
}
...
console.log(allPass
  ? `전 프리셋 통과 ✓ — ${rows.length}런 × 검사 전부 ✗0. 엔진이 브랜드·중립·대비 변경에 정합적으로 적응.`
